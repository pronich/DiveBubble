...
func New(cfg config.Config, db *sql.DB) http.Handler {
	...
	diveLogSvc := divelog.NewService(divelog.NewRepository(db))
	...
	registerTripRoutes(mux, tripSvc, transportSvc, buddySvc, diveCenterSvc, profileSvc, authIssuer, pushSvc, accountSvc, messageSvc, publisher)
	...
	registerDiveLogRoutes(mux, diveLogSvc, authIssuer)
	...
	registerDiveCenterRoutes(mux, diveCenterSvc, identityRepo, profileSvc, authIssuer, emailSvc)
