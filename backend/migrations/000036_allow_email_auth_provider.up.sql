-- The "email" passwordless provider (see internal/auth/email.go's LoginOrRegisterByEmail)
-- was missed when these CHECK constraints were first written for google/apple only.
...
ALTER TABLE auth_identities ADD CONSTRAINT auth_identities_provider_check
    CHECK (provider = ANY (ARRAY['google'::text, 'apple'::text, 'email'::text]));
