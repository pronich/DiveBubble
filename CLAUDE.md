...
| Data | `data/services/` | `TripApiService`, `ChatApiService`, `TransportApiService` (attach `Authorization: Bearer` via an injected `AccessTokenProvider`), `AuthApiService`/`TokenStorageService` (Google Sign-In token exchange + secure storage — see Auth section), `RealtimeService` (wraps a single shared `centrifuge.Client`, `subscribe`/`unsubscribe` per channel) |
...
| UI | `ui/features/profile/views/` | `ProfileView` (placeholder — just a Login button + signed-in check, no real profile fields yet) |
