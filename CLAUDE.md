...
- **Explore (done)**: `AppBar` title is left-aligned (`centerTitle: false`) and set in `headlineSmall` (Fraunces), not the default centered `titleLarge` — matches the serif treatment used on trip titles elsewhere. Card: 2-column grid, **4:3 landscape** photo (not square — an Airbnb-style crop reads less cramped) + gradient scrim + date pill, serif title (**1 line, ellipsis** — was 2 lines, wrapping made cards taller than needed), location, then exactly **two fixed badge rows**: row 1 is Depth/Duration/Dives (each individually omitted if its field is null, but never wraps — it's a `Row`, not a flexible `Wrap`), row 2 is always Level alone (`minCertification ?? 'Open to all'`, same fallback as Trip Page). Deliberately still **omits seats/price/type tags**, since capacity-as-a-cap and trip type aren't modeled yet and the platform isn't pricing trips (see Monetization above).
