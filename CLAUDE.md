...
Each of the 4 info tiles has a small leading icon: LEVEL → `Icons.badge_outlined` (cert card), DEPTH → `Icons.south` (down arrow — Material has no "arrow through a wave" glyph, so this is a deliberate simplification, not the literal ask), DIVES → `Icons.scuba_diving_outlined`, DURATION → `Icons.schedule`. The same 4 icons + compact abbreviated values (e.g. "15–30m", "3d", "2–5") appear as a `Wrap` of small badges on the Explore card too, below location — same icon set intentionally reused across both screens for visual consistency.
...
- **Explore (done)**: `AppBar` title is left-aligned (`centerTitle: false`) and set in `headlineSmall` (Fraunces), not the default centered `titleLarge` — matches the serif treatment used on trip titles elsewhere. Card: 2-column grid, square photo + gradient scrim + date pill, serif title (2 lines max), location, then a `Wrap` of small icon+value badges (LEVEL/DEPTH/DIVES/DURATION, same icon set as Trip Page's info grid, omitted per-badge when the field is null) — deliberately still **omits seats/price/type tags**, since capacity-as-a-cap and trip type aren't modeled yet and the platform isn't pricing trips (see Monetization above).
