...
- **Auth & onboarding**: animated first-run intro + login sheet (done, see App section below) — still UI-only, Google/Apple buttons are stubs. Real backend auth (JWT access + refresh-token sessions replacing stub `X-User-Id`, ForeignReader-style: `auth_identities`/`auth_sessions` tables, 8h access / 180d refresh TTL, refresh rotation) is the next slice, blocked on Google Cloud Console setup (Apple needs an App Store Connect app registration first, so Google ships first).
