...
- **Auth & onboarding**: animated first-run intro + login sheet (done, see App section below) — still UI-only, Google/Apple buttons are stubs. Backend Google auth (JWT access + refresh-token sessions, ForeignReader-style: `auth_identities`/`auth_sessions`, 8h access / 180d refresh TTL, refresh rotation — see Real auth backend section) is done and live but not yet wired to anything: the app doesn't call it, and existing routes still trust `X-User-Id`. Next: `google_sign_in` + secure storage + refresh interceptor in the app, then cut every route over from `withUser` to `bearerAuth` in one go. Apple needs an App Store Connect app registration first, so it's still after Google.
...
### Real auth backend (Google Sign-In, done — not yet wired into the app or into existing routes)
...
- **Google Cloud Console setup** (already done for DiveBuddy): one Web OAuth client (→ `GOOGLE_SERVER_CLIENT_ID`, no client secret needed for this flow) + one iOS OAuth client (bundle id `io.divebuddy.divebuddy`, → goes into the Flutter `google_sign_in` config once that's wired). Apple Sign-In is blocked on registering the app in App Store Connect first, so Google shipped first.
- **Still open**: nothing on the Flutter side consumes this yet (`LoginSheet`'s Google button is still a "Coming soon" stub) — next steps are wiring `google_sign_in` + secure token storage + a refresh-on-401 HTTP interceptor in the app, and then, separately, cutting every existing route over from `withUser`/`X-User-Id` to `bearerAuth` (deliberately not done gradually — there are no real users yet to migrate, so the plan is a single cutover rather than supporting both simultaneously long-term).
