...
*   **Models:** Define API/DTO models in `data/models/` with `freezed` + `json_serializable`. Suffix every data-model class name with `Model` (e.g. `MailFolderMetaModel`, `EmailApiModel`) so data models are always distinguishable from the suffix-free domain entities.
*   **Mappers:** Isolate the API-model → domain-entity transformation in `data/mappers/` as `extension` methods (`toDomain()`) on the API model, rather than inlining conversion in the Repository. Mappers live in the data layer (they reference API models) and must never leak into the domain.
*   **Repositories:** Consume one or more Services. Use Mappers to transform raw API models into clean domain entities. Handle caching, offline synchronization, and retry logic. Expose domain entities to ViewModels.
...
lib/
├── data/
│   ├── models/         # API/DTO models (freezed + json_serializable)
│   ├── mappers/        # API model -> Domain entity transforms (toDomain())
│   ├── repositories/   # Repository implementations
│   └── services/       # API clients, local storage wrappers
├── domain/
│   ├── entities/       # Clean domain entities (freezed)
...
### Task Progress
- [ ] **Step 1: Define Domain Entities.** Create immutable data classes for the feature in `domain/entities/` using `freezed` or `built_value`.
- [ ] **Step 2: Implement Services & API Models.** Create or update Service classes to handle external API communication, and their `freezed` API/DTO models in `data/models/`.
- [ ] **Step 3: Implement Mappers & Repositories.** Add `toDomain()` mappers in `data/mappers/`, then create the Repository to consume Services and return domain entities.
...
### Data Layer: Model, Mapper, Service and Repository

```dart
// 1. API/DTO Model (data/models/) — mirrors the wire format, no domain logic.
@freezed
...
// 2. Domain Entity (domain/entities/) — clean, framework-agnostic.
@freezed
abstract class User with _$User {
  const factory User({required String id, required String name}) = _User;
}

// 3. Mapper (data/mappers/) — API model -> Domain entity, as an extension.
extension UserApiMapper on UserApiModel {
  User toDomain() => User(id: id, name: fullName);
}

// 4. Service (Raw API interaction)
class ApiClient {
...
// 5. Repository (Single source of truth, returns domain entity via the mapper)
class UserRepository {
  ...
  Future<User> getUser(String id) async {
    ...
    _cachedUser = apiModel.toDomain(); // Transform via mapper
    return _cachedUser!;
  }
}
```
